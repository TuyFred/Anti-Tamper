  ...
  return (
    <div className="space-y-5 sm:space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              {['in_transit', 'rider_assigned'].includes(activeTransit.status) && activeTransit.unlock_token && (
                <Link
                  to="/deliveries"
                  className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-warning/10 text-warning border border-warning/25 text-sm font-semibold hover:bg-warning/20 transition"
                >
      ...
      {activeTransit && ['in_transit', 'rider_assigned'].includes(activeTransit.status) && (
        <DashboardPanel
          title="Live box tracking"
          subtitle="GPS updates automatically when your Smart Box is online"
        >
