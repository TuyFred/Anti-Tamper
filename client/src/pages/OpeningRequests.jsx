...
export default function OpeningRequests() {
  ...
  const handleApproveOpening = async (id) => {
    ...
    try {
      ...
      setSuccess(result?.message || 'Opening approved — unlock code sent to customer.');
      await refresh(true);
    } catch (err) {
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Customers asking to open the Smart Box again — not payment requests · {openingRequests.length} pending · 10 per page
        </p>
      </div>
                ...
                <article key={d.id} className="glass-card rounded-xl p-4 sm:p-5 border border-warning/25 space-y-3">
                  ...
                  <div className="p-3 rounded-xl bg-warning/5 border border-warning/25 text-xs text-slate-300 space-y-2">
                    ...
                    <p className="text-slate-400">
                      Customer closed the box or code expired and wants to open again.
